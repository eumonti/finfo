...
/* NOTA IMPORTANTE: i commenti non sono completi perch� si fa riferimento al testo del tema d'esame, qui di seguito riportato.

Un testo e' rappresentato come array di 100 caratteri. I caratteri utili nell'array sono quelli che precedono il (primo) carattere '\0'. Dati due testi, si vuole identificare la pi� lunga sequenza S di caratteri consecutivi (tra i quali possono comparire eventuali spazi) che e' contenuta in entrambi i testi.
Ad esempio, dati i testi "alternativamente" e "eternita", la pi� lunga sequenza contenuta in entrambi e' "tern" di lunghezza 4, mentre le sequenza "te" -anch'essa contenuta in entrambi- e' di lunghezza 2.  Dunque S sar� la sequenza "tern".
Detta L la lunghezza della sequenza S, se esistono pi� sequenze di lunghezza L  comuni ai due testi si utilizzi un criterio qualsiasi per scegliere una sola tra esse. 
...
/* massima lunghezza che ciascuno dei due testi da confrontare pu� avere,
 ...
 * di char contenente una sequenza di zero o pi� caratteri seguiti da un 
 * carattere '\0', usato come TERMINATORE. Perci� se la stringa da memorizzare
 ...
 * e a loro volta i loro valori di uscita rispettano tale convenzione. Ci� vale
...
int main()
{
  ...
  /* indice (in TestoA) del carattere iniziale del testo comune pi� lungo
   * trovato finora */
  int MaxLunghezza;
  /* lunghezza del pi� lungo testo comune ai due testi originali finora
   * trovato */
  int LunghezzaLocale;
  ...
  /* finch� non si e' raggiunta la fine del testo A */
  {
    CursoreB = 0;

    while ( '\0' != TestoB[CursoreB] )
		/* finch� non si e' raggiunta la fine del testo B */
		{
      ...
      /* se i caratteri correnti dei due testi sono uguali... */
      {
        ...
        LunghezzaLocale = 1;
        /* lo inizializziamo a 1 (e non a 0) poich� abbiamo gi� verificato
         * l'esistenza di un testo comune lungo 1 carattere */
        CursoreALocale = CursoreA+1;
        CursoreBLocale = CursoreB+1;
        /* posizioniamo i due cursori subito dopo il carattere comune gi�
         * trovato */
        ...
        (TestoA[CursoreALocale] == TestoB[CursoreBLocale]) )
        {
	      ...
	      /* se il testo comune appena trovato e' il pi� lungo trovato
	       * finora */
	      {
